...
public class Board implements IBoard {
    ...
    public IConveyorBelt getConveyorBelt(int x, int y) {
        ...
        switch (conveyorType) {
            case "straight":
                conveyorBelt = new ConveyorStraight(moveDirection, moveValue, position);
                break;
            case "turn":
            ...
            case "into":
                ...
                break;
            case "combine":
                conveyorBelt = new ConveyorCombine(moveDirection, moveValue, position);
                break;
            default:
                conveyorBelt = null;
        }
    ...
    public BoardElement getBoardElement(int x, int y) {
        ...
        switch (type) {
            case "conveyorBelt":
                elementType = BoardElement.CONVEYORBELT;
                break;
            case "hole":
                elementType = BoardElement.HOLE;
                break;
            case "wrench":
                String name = properties.get("name").toString();
                switch (name) {
                    case "normal":
                        elementType = BoardElement.WRENCH;
                        break;
                    case "hammer":
                        elementType = BoardElement.SPECIAL_WRENCH;
                        break;
                    case "flag1":
                        elementType = BoardElement.FLAG1;
                        break;
                    case "flag2":
                        elementType = BoardElement.FLAG2;
                        break;
                    case "flag3":
                        elementType = BoardElement.FLAG3;
                        break;
                    case "flag4":
                        elementType = BoardElement.FLAG4;
                        break;
                }
                break;
            case "gear":
